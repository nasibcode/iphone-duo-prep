...
| Autofix | **None.** Camera findings are `product-decision` only — never forced by `duo-harness autofix`. |
...
4. Re-run `swift run duo-harness audit …` — R5 findings should clear once hooks appear on the flagged lines (or accept as intentional product debt).
